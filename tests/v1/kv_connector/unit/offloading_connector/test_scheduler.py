...
@pytest.mark.parametrize("async_scheduling", [True, False])
def test_without_do_remote_decode_stores_only_new_blocks(
    request_runner, async_scheduling: bool
):
    """Without do_remote_decode, after loading prefix blocks from CPU,
    only the newly computed blocks are stored — the loaded prefix is
    skipped because next_stored_block_idx advances past it."""
    offloaded_block_size = 12
    gpu_block_size = 4
    num_gpu_blocks = 100
    ...
    # New request WITHOUT do_remote_decode, same 2 offloaded blocks.
    runner.new_request(token_ids=[0] * offloaded_block_size * 2)
    runner.connector_scheduler._maximal_prefix_lookup = lambda key, req_context: 1
    runner.manager.prepare_store.side_effect = (
        lambda keys, req_context: generate_store_output(keys)
    ...
    # Only the newly computed block (3, 4, 5) is stored —
    # the loaded prefix (0, 1, 2) is skipped.
    runner.run(
        decoded_tokens=[EOS_TOKEN_ID],
        expected_stored_gpu_block_indexes=(3, 4, 5),
    )
